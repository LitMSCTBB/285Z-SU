 ...
 * @param fEpsilonInner lower threshold for error to be accumulated (for the "I"
 *term to change, the measured error must be GREATER than this value)
 * @param fEpsilonOuter upper threshold for error to be accumulated (for the "I"
 *term to change, the measured error must be LESS than this value )
 ...
 **/
PID pidInit(float fKP, float fKI, float fKD, float fKF, float pInner,
 ...
 * @param uniDirectional (optional) set the PID controller to only output
 *positive values
 *
 * @return Output value constrained from @param min to @param max
 **/
float pidCalculate(PID &pid, float fSetPoint, float fProcessVariable, float min,
                   float max, bool uniDirectional) {
  ...
  float fDeltaE = 0;
  if (fError > pid.dInner && fError < pid.dOuter)
    fDeltaE = (fError - pid.lastError) / fDeltaTime;
  else
    fDeltaE = 0;

  pid.lastError = fError;

  if (fError > pid.EpsilonInner && fError < pid.EpsilonOuter)
    pid.sigma += fError * fDeltaTime;
  else
    pid.sigma = 0;

  if (fError < pid.pInner || fError > pid.pOuter)
    fError = 0;

  float fOutput = (fError * pid.kP      // add proportional
                   + pid.sigma * pid.kI // add integral
                   + fDeltaE * pid.kD   // add derivative
                   + fSetPoint * pid.kF // add base value
  );
  t1++;
  /*if (t1 % 10 == 0)*/ printf("%11.5f + %11.5f + %11.5f + %11.5f | \n", fError * pid.kP,
                               pid.sigma * pid.kI, fDeltaE * pid.kD, pid.kF * fSetPoint);
  fOutput = (fOutput > max) ? max : ((fOutput < min) ? min : fOutput);
  return (fOutput > 0 || (!uniDirectional && fOutput < 0)) ? fOutput : 0;
}

float pidCalculateWithE(PID &pid, float err, float fSetPoint,
  ...
  t1++;
  /* if (t1 % 10 == 0) */ printf("%11.5f + %11.5f + %11.5f | \n", fError * pid.kP,
                                 pid.sigma * pid.kI, fDeltaE * pid.kD);
  fOutput = (fOutput > max) ? max : ((fOutput < min) ? min : fOutput);
  return (fOutput > 0 || (!uniDirectional && fOutput < 0)) ? fOutput : 0;
}

/* Tune PID below */
PID flywheelPID = pidInit(95, 2, 0.15, 6.45, -FLT_MAX, FLT_MAX, -200, 200, -50,
                          200); // error units are RPM
PID drivePID = pidInit(1000, 400, 85, 0, -FLT_MAX, FLT_MAX, -5, 5, -13.5,
                       13.5); // error units are inches
PID turnPID =
    pidInit(800, 9000, 60, 0, -FLT_MAX, FLT_MAX, -1.75, 1.75, -40, 40);